...
- **Entry**: `src/main.tsx` (wraps `App` in `BrowserRouter`) → `App.tsx` (`<Routes>`: `/` → `src/components/index.tsx`'s `Index`, `/series/:slug` → `SeriePage.tsx`). `Index` renders one section component per screen area in a fixed order (Header, Hero, Artista, Spotify, Colecao, Series, Instagram, Conexoes, Footer) inside a single `<main>`. `SeriePage` renders `Header` + `Footer` around a single filtered `<Series only={nome} />`, so a série has both an in-page card (on `/`) and its own shareable URL.
...
- **Dormant components**: `CircularGallery.tsx` (a full OGL/WebGL infinite carousel with drag/wheel/touch, custom shaders, bent-plane layout), `TiltedCard.tsx` (a mouse-tilt card effect), and `ArtistaCard.tsx` are not imported anywhere. `Carrosel.tsx` is imported in `index.tsx` but its usage is commented out. Treat all of these as available-but-dormant building blocks, not dead code to delete without checking with the user first.
