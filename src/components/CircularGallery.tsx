...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 1.1 + cos(p.y * 2.0 + uTime) * 1.1) * (0.1 + uSpeed * 0.9);
  ...
  onResize({ screen, viewport }: { screen?: Screen; viewport?: Viewport } = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (1050 * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (1575 * this.scale)) / this.screen.width;
